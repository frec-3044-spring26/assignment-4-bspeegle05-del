...
```{r}
library(readxl)
iceDat <- read_excel(file_name, sheet = "data")
...
```{r}
sunapee <- iceDat %>% filter(lake_name == "Sunapee")
```
...
**Answer 1:**
I had to do a type conversion for the `ice_off_doy` column because it was originally a character type (peak readin imo)
```{r}
sunapee$ice_off_doy <- as.numeric(sunapee$ice_off_doy)
```

```{r}
sunapee %>% ggplot(aes(year,ice_off_doy)) + 
  geom_point() + 
  geom_line(color = "red") + 
  theme_classic() + 
  labs(x = "Year", y = "Date of Ice Off", title = "Date of Ice Off vs. Year")
```
...
**Answer 2:**
I expect the slope of the relationship to negative because as the years go on in the plot, there is a general downward trend in the date of ice off.
...
The direction of the slope matches the expectations I had from question 2.
...
```
The R^2 value is ~0.205, which means that 20.5% of the datapoints can be explained by the line of best fit. Because of this, it would be incorrect to say that there is a relation between the year and the date of ice off with the given data. 
...
```{r}
sunapee %>% ggplot(aes(residual)) + geom_histogram(binwidth = 10, fill = "yellow", color = "black") + theme_classic() + labs(x = "Residual", y = "Count")
```
The data is generally centered around 0, and it seems to be normally distributed.
...
```{r}
-7/fit$coefficients[2]
```
It would take about 66 years for the ice off day to have a one-week (7-day). This is meaningful because as time goes on, the ice off day is decreasing, which means it happens sooner. This could be because the global average temperature is increasing, which in turn causes the ice cover of lakes and rivers to melt sooner.
...
I am not comfortable with using this linear model for additional anaylsis. Although the p-value for the slope is very small, the R^2 value implies that the model will not be able to effectively predict future ice off dates.
...
sunapee %>% ggplot(aes(year,ice_off_doy)) + 
  ...
  annotate("text", x = 1890, y = 80, label = paste("y=",as.character(round(fit$coefficients[2],3)),"x","+",as.character(round(fit$coefficients[1],3)),sep = "")) + 
...
2040 Ice-Off Prediction
```{r}
predictValue <- 2040*fit$coefficients[2]+fit$coefficients[1]
```

```{r}
format(as.Date(2040*fit$coefficients[2]+fit$coefficients[1]),format = "%m-%d")
```

Confidence Interval 
```{r}
percenTiles <- quantile(sunapee$residual, probs = c(0.05,0.95))
```
With this, the confidence interval around the ice off date for 2040 is
```{r}
paste("[",
      as.character(round(predictValue + percenTiles[1], 3)),
                   ",",
                   as.character(round(predictValue - percenTiles[2], 3)),"]", 
                   sep = "")
```
Confidence Interval Width
```{r}
abs(percenTiles[1]) + abs(percenTiles[2])
```
...
```{r}
oldDat <- filter(sunapee, year <= 1950)
oldLm <- lm(oldDat$ice_off_doy ~ oldDat$year)
newDat <- filter(sunapee, year > 1950)
newLm <- lm(newDat$ice_off_doy ~ newDat$year)

sunapee %>% ggplot(aes(year,ice_off_doy)) + 
  geom_point() + 
  geom_abline(slope = newLm$coefficients[2], intercept = newLm$coefficients[1], color = "blue") + 
  annotate("text", x = 1890, y = 80, label = paste("y=",as.character(round(fit$coefficients[2],3)),"x","+",as.character(round(fit$coefficients[1],3)),sep = "")) + 
  labs(x = "Year", y = "Ice Off Date") + 
  theme_classic() + 
  geom_abline(intercept = oldLm$coefficients[1],slope = oldLm$coefficients[2], color = "red")

newLm$coefficients[2]
oldLm$coefficients[2]
```
With the red line representing all of the data before 1950 and the blue representing all of the data after 1950, it is clear that post-1950, the ice off day is decreasing slightly faster. Furthermore, the slopes of the lines support this, with about a 0.02 difference in slope. However small, this is very important, as this trend could eventually cause the ice off date to lower more quickly. Further exploration into this would entail including data from earlier than the dataset given, as it would allow for there to be a clearer relationship between pre-industrialization and post-industrialization ice off dates.
